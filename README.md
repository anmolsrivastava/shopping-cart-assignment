...
* When you have completed the assignment, you need to  push your code to this repository and [mark the assignment as completed by clicking here](https://app.snapcode.review/submission_links/9e1237ed-09e4-4b32-8de0-f0430e6dcd88).
...
1. **Product Price Source**:
   - The product prices are fetched from an external API in the format `{ "price": <price_value> }`.
   - If the API changes, the format should be updated in the `ProductApiClient` class.
   - Errors during price fetching will throw an error message 'Product not found'.

2. **Rounding Rules**:
   - The solution uses standard rounding to two decimal places for subtotal, tax, and total calculations.
   - If a different rounding method is required, modify the rounding logic in the `Cart` class.

3. **Tax Calculation**:
   - The tax rate is set to a fixed 12.5%.

4. **Extensibility**:
   - The solution supports a fixed set of products. To add new products, update the product catalog in the external API.
